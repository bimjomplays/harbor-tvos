...
- P8 switch source in place (M), and with it S5's player half: duration-mismatch-chip's "Find closer match" opens that in-place switcher, and the chip's buttons need a TV focus design.
...
- P11 exit snapshot (M), S4 subtitle step (M, off by default).
