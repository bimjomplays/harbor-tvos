...
**Status 2026-09-23:** Stage 0 ✔, Stage 1 ✔, Stage 2 ✔ (services band + service pages, card marks, collections), Stage 3 ✔ (detail page incl. TMDB cast/crew/similar/collection rows, episodes, ranked picker, debrid resolve, playback), Stage 4 partial (player chrome, resume/progress, online subtitles, audio/subtitle tracks, up-next; skip-intro, shaders, AVPlayer engine, display-mode matching pending), Stage 5 partial (Library rails, Addons manager, watchlist), Stage 7 mostly ✔ (Anime room: spec rows, CW, hero, awards, addon rows; MAL/AniList rails pending sign-in), Stage 8 mostly ✔ (M3U/Xtream sources, favorites, XMLTV guide grid; catch-up/VOD pending), Stage 9 partial (Big Picture settings catalog + commit; themes/desktop-only panels pending), Stage 11 MVP ✔ (ESPN hub, rows, hero, detail, personalize; watch matching pending). Profile sync now runs both ways on upstream's own engine inside the bundle (2026-09-23): roster, home/anime/nav/services rows, TV settings, watched-by; the account session is engine-owned too.
