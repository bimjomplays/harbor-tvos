// Port of upstream src-tauri/src/torrent_engine/cache_sweep.rs: the stream cache sweep (retention
// hours, then a size cap oldest-first). tvOS changes: `skip` names the folders of torrents the
// session still holds (never swept from under a live stream), and the tests use no uuid crate.
...
pub fn run_with_cancel(
    ...
    for entry in entries {
        ...
        let keep_name = path
            .file_name()
            .and_then(|n| n.to_str())
            .map(|n| KEEP.contains(&n) || skip.contains(n))
            .unwrap_or(false);
        if keep_name {
            continue;
        }
...
mod tests {
    ...
    fn sweep_reports_deleted_bytes_and_preserves_engine_files() {
        ...
        let stats = run_with_cancel(&dir, 0, 0, &HashSet::new(), &cancelled);
