// Track radio (Stage 12, second batch): lib/music/radio.ts, the "Start radio" station built
// around one track, re-ranked the same way, and extended while it plays (armTrackRadio).
// Differences on the TV, each forced by what the TV has:
//   - the YouTube lane is left out (no YouTube source on the TV, docs/music-spec.md);
//   - searchMusic (the music_search command) is musicSources.searchTracks;
//   - getMusicState()'s recents / liked come in as `familiar` from engine/music.ts;
//   - armTrackRadio's queue watcher lives in MusicPlayer.swift, which calls `extend` when the
//     queue runs within EXTEND_AT entries of its end.
// Upstream has no internet-radio (station stream) source; this is its only "radio".
import { safeFetch } from "@/lib/safe-fetch";
import { getSecret } from "@/lib/secret-store";
import { LASTFM_API_KEY } from "@/lib/music/lastfm";
import { loadRecordingProfile } from "@/lib/music/recording-profile";
import { artistCreditParts } from "@/lib/music/search-artists";
import { normalizeName, normalizeTitle } from "@/lib/music/search-normalize";
import type { MusicTrack } from "@/lib/music/types";
import { searchTracks } from "./musicSources";

type Obj = Record<string, unknown>;
type Lane = "lastfm" | "deezerRadio" | "related";
...
type Candidate = { track: MusicTrack; key: string; deezerId?: number; lanes: Map<Lane, number>; features?: Features; score: number };

const STATION_SIZE = 40;
const EXTEND_SIZE = 18;
export const EXTEND_AT = 4;
const LANE_MS = 7000;
const FEATURE_LOOKUPS = 36;
const LANE_WEIGHT: Record<Lane, number> = { lastfm: 1, deezerRadio: 0.7, related: 0.6 };
const VARIANT = /\b(karaoke|instrumental|tribute|cover|sped ?up|slowed|nightcore|8d|reverb|remix|live at|acoustic version|piano version|ringtone|made popular|originally performed|in the style of)\b/i;
...
const image = (...values: unknown[]): string => values.map(text).find((value) => value.startsWith("https://")) ?? "";
...
async function deezer(path: string): Promise<Obj> {
  ...
  const request = (async () => {
    const response = await bounded(safeFetch(`https://api.deezer.com/${path}`, { headers: { Accept: "application/json" } }));
...
async function lastfm(params: Record<string, string>): Promise<Obj | null> {
  ...
  const response = await bounded(safeFetch(`https://ws.audioscrobbler.com/2.0/?${query}`, { headers: { Accept: "application/json" } }));
  if (!response.ok) return null;
  const value = obj(await response.json());
  return value.error ? null : value;
}

export function trackKey(track: Pick<MusicTrack, "title" | "artist">): string {
...
async function trackFeatures(deezerId: number): Promise<Features> {
  ...
  return {
    ...
    genres: rows(obj(album.genres).data).map((entry) => num(entry.id) ?? 0).filter(Boolean),
  };
}
...
async function resolveSeed(track: MusicTrack): Promise<Seed> {
  ...
  const features = deezerId ? await trackFeatures(deezerId).catch(() => ({ genres: [] })) : { genres: [] };
...
async function deezerRadioLane(seed: Seed): Promise<[MusicTrack, number][]> {
  ...
  const list = rows((await deezer(`artist/${seed.artistId}/radio?limit=40`)).data).map(deezerTrack).filter((track): track is MusicTrack => !!track);
...
async function lastfmLane(seed: Seed): Promise<[MusicTrack, number][]> {
  ...
  const value = await lastfm({ method: "track.getSimilar", artist: lead, track: seed.title, limit: "60" });
  const similar = rows(obj(value?.similartracks).track)
    .map((entry) => ({ title: text(entry.name), artist: text(obj(entry.artist).name), match: num(entry.match) ?? 0 }))
    .filter((entry) => entry.title && entry.artist && entry.match > 0.05)
    .slice(0, 16);
  const found = await Promise.all(
    similar.map(async (entry) => {
      const hits = await bounded(searchTracks(`${entry.artist} ${entry.title}`, 4), 5000).catch(() => [] as MusicTrack[]);
      const wanted = `${normalizeTitle(entry.title)}|${normalizeName(entry.artist)}`;
      const exact =
        hits.find((hit) => trackKey(hit) === wanted) ??
        hits.find((hit) => normalizeTitle(hit.title) === normalizeTitle(entry.title) && normalizeName(hit.artist).includes(normalizeName(entry.artist)));
...
function affinity(seed: Features, candidate: Features | undefined): number {
  ...
  if (seed.bpm && candidate.bpm) {
    const delta = Math.min(Math.abs(seed.bpm - candidate.bpm), Math.abs(seed.bpm - candidate.bpm * 2), Math.abs(seed.bpm * 2 - candidate.bpm));
    bonus += 0.15 * (1 - Math.min(1, delta / 25));
  }
  if (seed.year && candidate.year) bonus += 0.08 * (1 - Math.min(1, Math.abs(seed.year - candidate.year) / 12));
  if (seed.gain !== undefined && candidate.gain !== undefined) bonus += 0.05 * (1 - Math.min(1, Math.abs(seed.gain - candidate.gain) / 6));
...
async function gather(seeds: Seed[], exclude: Set<string>): Promise<Candidate[]> {
  ...
  const settled = await Promise.allSettled(seeds.flatMap((seed) => lanes.map(async ([lane, run]) => [lane, await bounded(run(seed))] as const)));
  for (const result of settled) {
    ...
    for (const [track, position] of list) {
      ...
      const entry = pool.get(key) ?? { track, key, deezerId: deezerNumeric(track.id, "track"), lanes: new Map(), score: 0 };
      ...
      if (entry.track.connectorId === "catalog" && track.connectorId && track.connectorId !== "catalog") entry.track = track;
      pool.set(key, entry);
    }
  }
  return [...pool.values()];
...
function leadOf(track: MusicTrack): string {
  return normalizeName(artistCreditParts(track.artist)[0] ?? track.artist);
}

function rank(seed: Seed, candidates: Candidate[], familiarTracks: MusicTrack[]): Candidate[] {
  const familiar = new Set(familiarTracks.map(trackKey));
  const seedLead = leadOf(seed);
  for (const candidate of candidates) {
    ...
    if (leadOf(candidate.track) === seedLead) score -= 0.15;
  ...
  let familiarCount = 0;
  for (const candidate of candidates) {
    const lead = leadOf(candidate.track);
    const seen = perArtist.get(lead) ?? 0;
    const known = familiar.has(candidate.key);
    if (seen >= (ordered.length < 30 ? 2 : 3) || (known && familiarCount >= Math.ceil(STATION_SIZE / 5))) {
  ...
  while (rest.length) {
    ...
    const previousLead = previous ? leadOf(previous.track) : "";
    const index = rest.findIndex((candidate) => leadOf(candidate.track) !== previousLead);
    spaced.push(rest.splice(index < 0 ? 0 : index, 1)[0]!);
  }
  return spaced;
}

async function build(seeds: Seed[], exclude: Set<string>, size: number, familiar: MusicTrack[]): Promise<MusicTrack[]> {
  const candidates = await gather(seeds, exclude);
  await enrich(candidates);
  return rank(seeds[0]!, candidates, familiar)
    .slice(0, size)
    .map((candidate) => ({ ...candidate.track, mediaKind: "audio" as const }));
}

/** radio.ts loadTrackRadio: the seed first, then at least five more, or music.radio.error. */
export async function loadTrackRadio(track: MusicTrack, familiar: MusicTrack[]): Promise<MusicTrack[]> {
 ...
 */
export async function extendTrackRadio(queue: MusicTrack[], index: number, familiar: MusicTrack[]): Promise<MusicTrack[]> {
  if (!queue.length || index < 0) return [];
  const recent = queue.slice(Math.max(0, index - 2), index + 1);
  const seeds = await Promise.all(recent.map(resolveSeed));
